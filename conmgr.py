...
class ConnectionManager(object):
    def __init__(self, application, mid, adr, db, usr, psw):
        ...
        self._APP.register_timer("reauth", self.authenticate, [],  interval=8000, service=True)
        ...
        self._APP.register_timer("markings_check", self.check_unsent_markings, [],  interval=20000, service=True) 
        self._APP.register_timer("db_rebase", self.db_init, [],  interval=600000, service=True)
    ...
    def authenticate(self):
        ...
        if self.uid:
            ...
            self._APP.register_timer("keepalive", self._keepalive, [], interval=1000, service=True)
    ...
    def check_unsent_markings(self):
        if self.haslocal:
            ...
            if bool(markings) and (self.status == 2):
                logger.info("Trovate %d marcature locali, invio..." % len(markings))
    ...
    def execute_kw(self, *args):
        if self.status != 2:
            raise xcl.socket.error("Connessione Assente...")

        try:
            return self.models.execute_kw(self.db, self.uid, self.psw, *args)
        except ValueError as e:
            raise xcl.socket.error(str(e))
    ...
    def register_marking(self, employee_id):
        ...
        except (xcl.Fault) as e:
            logging.exception(e)
            logging.critical("Errore nella registratura!")
            pass
